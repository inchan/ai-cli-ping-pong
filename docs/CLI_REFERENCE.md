...
MCP 도구(`run_tool`)를 통해 자동으로 처리됩니다.
...
**Arguments**:
- `cli_name` (string, required): `list_tools`로 조회된 CLI 이름
...
**Arguments**: `run_tool`와 동일
**Returns**: `{"task_id": "..."}`

### 4. `get_run_status`
`start_run_tool`로 시작된 비동기 작업의 현재 상태를 조회합니다.
...
3.  **결과 수신**
    작업이 완료되면, `get_run_status` 호출은 다음과 같은 최종 결과를 반환합니다.
    ```json
    {
      "status": "completed",
...
|------|---------|---------|
| 2025-12-03 | 1.1.0 | 비동기 작업 도구(`start_run_tool`, `get_run_status`) 추가 및 문서 개편 |
