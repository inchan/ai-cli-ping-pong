...
기존의 `send_message` 도구는 동기적으로(synchronously) 동작하여, AI CLI의 작업이 끝날 때까지 클라이언트의 요청을 차단(blocking)하는 문제가 있습니다. 이로 인해 수 분이 소요되는 긴 작업의 경우, 클라이언트는 응답이 올 때까지 아무것도 하지 못하고 기다려야 합니다.
...
**작동 흐름:**
1.  클라이언트는 `start_send_message`를 호출하여 긴 작업을 요청합니다.
...
### 3.1 `start_send_message`

긴 작업을 시작하고 즉시 `task_id`를 반환합니다.

-   **Input Schema:**
    -   기존 `send_message`의 모든 파라미터 (`cli_name`, `message`, `system_prompt`, `args`, `timeout` 등)를 동일하게 사용합니다.
-   **Output Schema (즉시 반환):**
    ```json
    {
