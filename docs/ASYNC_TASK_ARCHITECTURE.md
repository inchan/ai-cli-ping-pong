...
기존의 `run_tool` 도구는 동기적으로(synchronously) 동작하여, AI CLI의 작업이 끝날 때까지 클라이언트의 요청을 차단(blocking)하는 문제가 있습니다. 이로 인해 수 분이 소요되는 긴 작업의 경우, 클라이언트는 응답이 올 때까지 아무것도 하지 못하고 기다려야 합니다.
...
**작동 흐름:**
1.  클라이언트는 `start_run_tool`를 호출하여 긴 작업을 요청합니다.
2.  서버는 즉시 작업을 백그라운드에서 시작하고, 해당 작업을 추적할 수 있는 고유한 `task_id`를 클라이언트에게 즉시 반환합니다.
3.  클라이언트는 받은 `task_id`를 사용하여, 원하는 시점에 `get_run_status`를 주기적으로 호출하여 작업의 진행 상태를 확인(Polling)합니다.
4.  작업이 완료되면, `get_run_status`의 응답에 최종 결과 또는 에러가 포함됩니다. 클라이언트는 이 응답을 받고 폴링을 중단합니다.
...
-   **Input Schema:**
    -   기존 `run_tool`의 모든 파라미터 (`cli_name`, `message`, `system_prompt`, `args`, `timeout` 등)를 동일하게 사용합니다.
-   **Output Schema (즉시 반환):**
    ```json
    {
...
-   **Input Schema:**
    ```json
    {
      "task_id": {
        "type": "string",
        "description": "start_run_tool로부터 받은 작업 ID"
      }
    }
    ```
-   **Output Schema:**
